...
from picard.options import LocalCoverMatchMode
...
class ScriptToGlobMatchTest(LocalCoverArtTestBase):
    ...
    def test_script_with_conditional(self):
        """Script uses $if to pick a base name and {jpg,png} alternation for extensions."""
        self._create_files(
            [
                'Abbey Road.jpg',
                'Abbey Road.png',
        ...
        script = '$if($eq(%album%,Abbey Road),%album%,cover).{jpg,png}'
    ...
    def test_reporter_shared_cover_directory(self):
        """Regression test for the PICARD-1044 reporter's real-world scenario.

        Multiple "singular track" covers live in one shared Cover/ directory,
        so directory scoping alone cannot disambiguate them; the script-built
        file name (with the example pattern) must select exactly the right one.
        Uses the actual file names from the reporter's comment on PR #3042.
        """
        cover_dir = os.path.join(self.tmpdir, 'Cover')
        os.makedirs(cover_dir)
        filenames = (
            'Alicia Keys - The Diary Of Alicia Keys [2003].jpg',
            'The Ting Tings - We Started Nothing [2008].jpg',
            'Various Artists - Monstercat 001 - Launch Week [2011].jpg',
            'Various Artists - Spotlight Compilation Vol. 1 [2013].jpg',
        )
        for name in filenames:
            open(os.path.join(cover_dir, name), 'w').close()

        # The example pattern shipped in the SCRIPT-mode help.
        script = '%albumartist% - %album%$if(%date%, [%date%],).{jpg,png}'
        cases = (
            (
                {'albumartist': 'Alicia Keys', 'album': 'The Diary Of Alicia Keys', 'date': '2003'},
                'Alicia Keys - The Diary Of Alicia Keys [2003].jpg',
            ),
            (
                {'albumartist': 'The Ting Tings', 'album': 'We Started Nothing', 'date': '2008'},
                'The Ting Tings - We Started Nothing [2008].jpg',
            ),
            (
                {'albumartist': 'Various Artists', 'album': 'Monstercat 001 - Launch Week', 'date': '2011'},
                'Various Artists - Monstercat 001 - Launch Week [2011].jpg',
            ),
            (
                {'albumartist': 'Various Artists', 'album': 'Spotlight Compilation Vol. 1', 'date': '2013'},
                'Various Artists - Spotlight Compilation Vol. 1 [2013].jpg',
            ),
        )
        for tags, expected in cases:
            results = self._find_with_script(script, Metadata(tags))
            # Exactly the one intended file matches, despite sharing the directory.
            self.assertEqual([expected], self._filenames(results))


class QueueImagesModeSelectionTest(PicardTestCase):
    """The active mode selects which per-mode value is read and how it is evaluated."""

    def setUp(self):
        super().setUp()
        self.provider = CoverArtProviderLocal.__new__(CoverArtProviderLocal)
        self.provider._queue_images_regex = MagicMock()
        self.provider._queue_images_script = MagicMock()

    def test_regex_mode_reads_regex_value(self):
        self.set_config_values(
            setting={
                'local_cover_regex': 'cover',
                'local_cover_script': '%album%.*',
                'local_cover_match_mode': LocalCoverMatchMode.REGEX,
            }
        )
        self.provider.queue_images()
        self.provider._queue_images_regex.assert_called_once_with('cover')
        self.provider._queue_images_script.assert_not_called()

    def test_script_mode_reads_script_value(self):
        self.set_config_values(
            setting={
                'local_cover_regex': 'cover',
                'local_cover_script': '%album%.*',
                'local_cover_match_mode': LocalCoverMatchMode.SCRIPT,
            }
        )
        self.provider.queue_images()
        self.provider._queue_images_script.assert_called_once_with('%album%.*')
        self.provider._queue_images_regex.assert_not_called()

    def test_empty_active_value_queues_nothing(self):
        # Script mode active but the script value is empty: nothing is queued,
        # even though the (inactive) regex value is set.
        self.set_config_values(
            setting={
                'local_cover_regex': 'cover',
                'local_cover_script': '',
