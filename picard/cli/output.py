...
"""Colored terminal output for Picard CLI commands.

Provides structured output with optional ANSI color support, automatic
TTY detection, and semantic display methods for consistent formatting
...
class CliOutput:
    """Output wrapper for CLI commands with optional color support.

    Provides:
    - Basic output: print, info, success, error, warning, nl
    - Color helpers: bold, dim, red, green, yellow, blue, magenta, cyan
    - Semantic display: d_name, d_version, d_url, d_path, d_command, etc.
    - User interaction: yesno prompts
    """
    ...
    def __init__(self, stdout=None, stderr=None, color=None):
        ...
        Args:
            ...
            color: Enable color output. None = auto-detect from TTY,
                   True = force color, False = disable color.
        """
    ...
    def d_id(self, identifier):
        """Display identifier (plugin_id, profile_id, etc.)."""
    ...
    def d_arrow(self):
        """Display transition arrow (before → after)."""
