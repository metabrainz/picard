...
class ISRCSubmitEntry:
    ...
    def __init__(self, recording_id: str, new_isrcs: set[NormalizedISRC]):
...
class ISRCSubmitManager:
    ...
    def add(self, file, recording_id: str, file_isrcs: list[str], mb_isrcs: list[str]) -> None:
        ...
        new_isrcs: set[NormalizedISRC] = set()
    ...
    def remove_album(self, album) -> None:
        """Remove all pending ISRCs for an album (e.g., when album is removed)."""
        to_remove = [obj for obj in self._entries if getattr(obj, 'album', None) is album]
        if to_remove:
    ...
    def find_duplicate_isrcs(self) -> set[NormalizedISRC]:
        ...
        isrc_to_recordings: dict[NormalizedISRC, set[str]] = {}
    ...
    def check_track_submittable(
        track,
        new_isrcs: set[NormalizedISRC],
        existing_isrcs: list[str],
        duplicate_isrcs: set[NormalizedISRC],
        album_isrcs: dict[NormalizedISRC, str] | None = None,
    ) -> tuple[bool, str]:
        ...
        if new_isrcs:
            ...
            # Check if the ISRC is already assigned to another track on this release
            if album_isrcs:
                track_own_isrcs = normalized_isrcs(existing_isrcs)
                for isrc in new_isrcs:
                    if isrc in album_isrcs and isrc not in track_own_isrcs:
                        return False, N_("ISRC already assigned to another track on this release")
            return True, ''
        elif existing_isrcs:
