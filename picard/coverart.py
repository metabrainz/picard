...
def _coverart_downloaded(album, metadata, release, try_list, coverinfos, data, http, error):
    ...
    if error or len(data) < 1000:
        ...
        QObject.tagger.window.set_statusbar_message(N_("%s coverart downloaded for %s from %s"),
...
def _walk_try_list(album, metadata, release, try_list):
    ...
    if len(try_list) == 0:
        ...
        QObject.tagger.window.set_statusbar_message(N_("Downloading %s coverart for %s from %s ..."),
