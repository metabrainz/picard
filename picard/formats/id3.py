...
class ID3File(File):
    ...
    def _remove_single_tag(self, tags, name, config_params):
        ...
        try:
            if name.startswith('performer:'):
                ...
                self._remove_license_tag(tags, name)
            elif name == '~rating':
                self._remove_rating_tag(tags, name, config_params)
            elif name in self.__rtranslate:
                self._remove_translated_tag(tags, name)
            elif name.lower() in self.__rtranslate_freetext_ci:
                self._remove_freetext_ci_tag(tags, name)
            elif name in self.__rtranslate_freetext:
                self._remove_freetext_tag(tags, name)
                ...
                self._remove_other_supported_tag(tags, name)
        except KeyError:
            pass
    ...
    def _remove_license_tag(self, tags, name):
    ...
    def _remove_translated_tag(self, tags, name):
    ...
    def _remove_freetext_tag(self, tags, name):
    ...
    def _remove_other_supported_tag(self, tags, name):
