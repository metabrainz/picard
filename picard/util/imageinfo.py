...
def identify(data):
    ...
    if data[:6] in (b'GIF87a', b'GIF89a'):
        w, h, mime, extension = _gif_read(data)

    # http://en.wikipedia.org/wiki/Portable_Network_Graphics
    # http://www.w3.org/TR/PNG/#11IHDR
    elif data[:8] == b'\x89PNG\x0D\x0A\x1A\x0A' and data[12:16] == b'IHDR':
        w, h, mime, extension = _png_read(data)

    # http://en.wikipedia.org/wiki/JPEG
    elif data[:2] == b'\xFF\xD8':  # Start Of Image (SOI) marker
        w, h, mime, extension = _jpeg_read(data)

    # WebP
    elif data[:4] == b'RIFF' and data[8:12] == b'WEBP':
        w, h, mime, extension = _webp_read(data)

    # TIFF
    elif data[:4] == b'II*\x00' or data[:4] == b'MM\x00*':
        w, h, mime, extension = _tiff_read(data)

    # PDF
    elif data[:4] == b'%PDF':
        w, h, mime, extension = _pdf_read(data)
...
def _gif_read(data):
    ...
    return (w, h, 'image/gif', '.gif')


def _png_read(data):
    w, h = struct.unpack('>LL', data[16:24])
    return (w, h, 'image/png', '.png')
    ...
    try:
        ...
        for i in range(entry_count):
            ...
            if tag == TIFF_TAG_IMAGE_WIDTH:
                w = _tiff_read_value(type, order, field[8:12])
            elif tag == TIFF_TAG_IMAGE_LENGTH:
                h = _tiff_read_value(type, order, field[8:12])
            if w and h:  # Found both width and height, abort
                break
            pos += 12
    except struct.error:
        pass
    return (w, h, 'image/tiff', '.tiff')


def _pdf_read(data):
    return (0, 0, 'application/pdf', '.pdf')
